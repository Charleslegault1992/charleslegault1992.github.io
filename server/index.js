...
import { createPostgresPoolPressureMonitor } from "./persistence/postgresPoolPressureMonitor.js";
...
import { createPostgresRuntimePersistence } from "./persistence/postgresRuntimePersistence.js";

let postgresPoolPressureMonitor = null;

const startGameApplication = async () => {
  ...
  const authService = createHmacAuthService({
    secret: authSecret,
  });
  ...
  const persistence = await createPostgresRuntimePersistence({
    databaseOptions: getPostgresApplicationOptions(),
  });

  postgresPoolPressureMonitor = createPostgresPoolPressureMonitor({
    getPoolStats: () => persistence.getPoolStats(),
  });
  ...
  try {
    const { accountRepository, characterRepository, chatModerationRepository } = persistence;
    ...
    const chatModerationService = createChatModerationService({
      repository: chatModerationRepository,

      moderatorAccountIds,
    });

    const moderationInitialization = await chatModerationService.initialize(Date.now());
    ...
    const httpApi = createGameHttpApi({
      ...
      isCharacterOnline: (accountId, characterId) => runtime.isCharacterBusy(accountId, characterId),
    });
    ...
    const stop = async ({
      ...
      resetScheduler?.stop();
      postgresPoolPressureMonitor?.stop();

      const forceExitTimer = setTimeout(() => process.exit(exitCode || 1), 60000);
      ...
      try {
         ...
         * failed removals, etc.
         */
        const playerSaveResult = await runtime.saveAllPlayerPersistence();

        if (!playerSaveResult.success) {
          finalExitCode = finalExitCode || 1;

          console.error("Final player persistence failed:", playerSaveResult.failedPlayerUids);
        }
         ...
         * after server.stop().
         */
        const moderationSaveResult = await chatModerationService.flushPersistence();

        if (!moderationSaveResult.success) {
          finalExitCode = finalExitCode || 1;

          console.error("Final chat moderation persistence failed:", moderationSaveResult.failedAccountIds);
        }
        ...
        try {
          await persistence.close();
        } catch (persistenceCloseError) {
          console.error("PostgreSQL pool shutdown failed:", persistenceCloseError);
        }
     ...
     */
    await server.start();
    postgresPoolPressureMonitor.start();
    ...
    console.log(`PostgreSQL pool: ${poolStats.total} total, ${poolStats.idle} idle, ${poolStats.waiting} waiting.`);
    ...
    if (server) {
      try {
        ...
        console.error("Server cleanup after startup failure failed:", serverStopError);
      }
    }
    ...
    try {
      ...
      console.error("PostgreSQL cleanup after startup failure failed:", persistenceCloseError);
    }
...
try {
  ...
  console.error("Game server startup failed:", error);
  postgresPoolPressureMonitor?.stop();
